...
public class ClassesController implements Initializable {
    ...
    @FXML
    private TableColumn<model.Class, String> classLecturerCol;
    @FXML
    ...
    @FXML
    private ChoiceBox<Lecturer> newClassLecturer;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        classLecturerCol.setCellValueFactory(cellData -> cellData.getValue().classLecturer());
        ...
        newClassLecturer.setItems(PaneNavigator.getMainApp().getLecturersData());
    ...
    private void handleNewClass() {
        ...
        Lecturer lecturer = newClassLecturer.getValue();
        Room room = newClassRoom.getValue();

        if(classDataIsValid()){
            PaneNavigator.getMainApp().getClassData().add(new Class(course,type,lecturer,group,room));
        }
    }
    ...
    private void generateTable(ActionEvent event) {
        ...
        Data.setLecturers(PaneNavigator.getMainApp().getLecturersData().toArray(new Lecturer[0]));
